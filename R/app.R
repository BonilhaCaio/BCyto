...
initializing <- function() {
  ...
  ch <- vapply(seq(colnames(bc$gs)), function(x)
    paste(colnames(bc$gs)[x], bc$channelNames[x], sep=" :: "), character(1))
...
addYAxis <- function(Y, axis, font) {
  ...
  if(Y == "Time") {
    ...
    if(axis == TRUE) {
      ...
      title(ylab=names(bc$ch)[id], cex.lab=(1+font/10), line=2.6, font.lab=2)
    }
  }
}
...
newPlot <- function(ID, X, Y, parent, typ, axis, font, bgDF=NULL, bg=FALSE) {
  ...
  if(length(ff@exprs) > 0) {
    ...
    if(axis == TRUE) {
      ...
      title(xlab=names(bc$ch)[id], cex.lab=(1+font/10), line=lin, font.lab=2)
    }
  } else {
    alert(paste0("The selected sample has 0 events under the selected parent.
                 Please choose another sample or change the parent."))
  }
}
...
newDotPlot <- function(dfX, X, Y, typ, axis, font, bgDF, xLim) {
  ...
  if(typ == "Backgating") {
    ...
    if(typ != "Contour") {
      ...
      plot(dfX, dfY, xaxt="n", yaxt="n", ann=FALSE, pch=20, cex=0.5, lwd=0,
           col=density, xlim=xLim, ylim=yLim, xaxs="i", yaxs="i")
    }
...
gateLeg <- function(ID, X, Y, typ, show, font, bg, freq, coords, x, i) {
  ...
  if(length(bc$found) == 4) {
    ...
    if(typ != "Histogram") {
      ...
      legY <- axisYLim*0.95
      legend(legX, legY, title=title, legend=freq, cex=1+font/10, bg=col,
             box.lwd=0, x.intersp=-0.5, y.intersp=0.8, text.font=2, xjust=0.5)
    }
  }
}
...
ovHist <- function(IDs, dfsX, typ) {
  ...
  if(typ == "Offset histogram") {
    ...
    for(i in rev(seq(IDs))) {
      ...
      polygon(densLine, col=names(bc$colorList[i]), border=NA, lwd=1.5, lty=1)
...
ancestryGen <- function(typ, previewSamp, bgPop) {
  ...
  if(typ != "" && previewSamp != "") {
    ...
    data <- gs_pop_get_count_fast(bc$gs[[1]], path="auto")
...
prolifGen <- function(prolifSB, show, font, ready, lab, grid) {
  ...
  bc$prolifParent <- gs_pop_get_count_fast(bc$gs[bc$sampleList[1]],
                                           path="auto")
  ID <- which(bc$prolifParent[,2] == "undivided")
  ...
  tabsetPanel(
        ...
        column(
          ...
          sidebarPanel(
            ...
            shinyDirButton("directory", "Load FCS files",
                           ...
                           "Please select a folder with your FCS files"),
            br(),
            br(),
            strong("or"),
            br(),
            ...
            shinyFilesButton("BCytoFileLoad", "Load BCyto file",
                             icon=icon("file-upload"),
                             "Please select a BCyto file (it needs to be
                             located in the same folder of the FCS files)",
                             multiple=FALSE),
            br(),
            br(),
            strong("or"),
            ...
            br(),
            br(),
            actionButton(inputId="saveFile", label=div(icon("save"),
                                                       "Save")),
            br(),
            br(),
            ...
            fluidRow(
              column(
                width=3, offset=9,
                actionButton(inputId="fileHelp", icon("question-circle"))
              )
            ),
            width=12
          ),
    ...
    tabPanel(
      ...
      sidebarPanel(
        ...
        fluidRow(
          column(
            width=4, actionButton(inputId="displayOptMain",
                                  label=div(icon("bars"), "Display options"))
          ),
          column(
            width=2, offset=5, actionButton(inputId="plotHelp",
      ...
      mainPanel(
        ...
        fluidRow(
          column(
            width=3, div(style="display:inline-block; position:fixed",
                         actionButton(inputId="previousSample", width="40px",
                                      icon("chevron-left"), style=bc$blackB),
                         actionButton(inputId="nextSample", width="40px",
                                      icon("chevron-right"), style=bc$blackB) )
          ),
          column(
            width=5, offset=4, align="left", id="gatetools",
            div(style="display:inline-block; position:fixed",
                actionButton(inputId="rectang", width="40px",
                             img(src=dataURI(
                               file=system.file("icons", "Rect.png",
                                                package="BCyto"),
                               mime="image/png"), height = "14px"),
                             style=bc$blackB),
                actionButton(inputId="polygon", width="40px",
                             img(src=dataURI(
                               file=system.file("icons", "Polygon.png",
                                                package="BCyto"),
                               mime="image/png"), height = "14px"),
                             style=bc$blackB),
                actionButton(inputId="quad", width="40px",
                             img(src=dataURI(
                               file=system.file("icons", "Quad.png",
                                                package="BCyto"),
                               mime="image/png"), height = "14px"),
                             style=bc$blackB),
                actionButton(inputId="interval", width="40px",
                             img(src=dataURI(
                               file=system.file("icons", "Interval.png",
                                                package="BCyto"),
                               mime="image/png"), height = "7px"),
                             style=bc$blackB),
            )
          ),
          column(
        ...
        div(style="display:inline-block",
            downloadButton(outputId="saveMainPlot", label="Export image")),
      ...
      sidebarPanel(
        ...
        fluidRow(
          ...
          column(width=1,
                 actionButton(inputId="parentHelp", icon("question-circle"))
          )
        ),
        width=4
      )
    ...
    tabPanel(
      ...
      br(),
      sidebarPanel(
        selectInput(inputId="previewSample", label=div(icon("vial"),
                                                       "Preview sample"),
                    choices=c("")),
        checkboxInput(inputId="showUncomp",
                      label=strong("Overlay uncompensated"),
                      value=TRUE),
        selectInput(inputId="previewMatrix", label=div(icon("table"),
                                                       "Preview Matrix"),
                    choices=c("Cytometer-defined (applied)"=
                                "Cytometer-defined")),
        tags$style("#createMatrix {border-color:black}"),
        actionButton(inputId="createMatrix", label="Create New Matrix",
                     icon("plus-square")),
        br(),
        br(),
        tags$style(paste0("#saveMatrix {", bc$blackB, "}")),
        actionButton(inputId="saveMatrix", label="Save New Matrix",
                     icon("sd-card")),
        br(),
        br(),
        ...
        actionButton(inputId="applyMatrix", label="Apply to Samples",
                     icon("share-square")),
        width=3
      ),
      mainPanel(
    ...
    tabPanel(
      ...
      br(),
      sidebarPanel(
        selectInput(inputId="bgPreviewSample", label=div(icon("vial"),
                                                         "Sample"),
                    choices=c("")),
        selectInput(inputId="bgType", label=div(icon("chart-area"),
                                                "Plot type"),
                    choices=c("", "Pseudocolor", "Contour", "Density",
                              "Backgating"), selected=""),
        selectInput(inputId="bgPop", label=div(tags$i(class="fas fa-clone"),
                                               HTML("Backgating
    ...
    tabPanel(
      ...
      br(),
      sidebarPanel(
        selectInput(inputId="ovTyp", label=div(icon("chart-area"),
                                               "plot type"),
                    choices=c("", "Overlaid histogram", "Offset histogram",
                              "Dot plot")),
        selectInput(inputId="ovTon", label=div(icon("chart-area"), "Tone"),
                    choices=c("", "Colorful", "B&W")),
        selectInput(inputId="ovY", label=div(
          img(src=dataURI(file=system.file("icons", "Y.png", package="BCyto"),
                          mime="image/png"), height="15px"),
                                             "Y axis"), choices=c("")),
        selectInput(inputId="ovX", label=div(
          img(src=dataURI(file=system.file("icons", "X.png", package="BCyto"),
                          mime="image/png"), height="15px"),
                                             "X axis"), choices=c("")),
        selectInput(inputId="ovP", label=div(icon("sitemap"), "Parent"),
                    choices=c("", "ungated")),
        tags$style("#ovSamples {border-color:black}"),
        actionButton(inputId="ovSamples", label="Select samples",
                     icon("plus-square")),
        br(),
        br(),
        tags$style("#displayOptOv {border-color:black}"),
        actionButton(inputId="displayOptOv", label=div(icon("bars"),
                                                       "Display options")),
        width=3
      ),
      mainPanel(
        ...
        actionButton(inputId="ovSampleOrder", label="Edit order",
                     icon("fas fa-pen")),
    ...
    tabPanel(
      ...
      sidebarPanel(
        ...
        actionButton(inputId="step1", label=div("Step 1: gate undivided",
                                                tags$i(class="far
                                                       fa-question-circle")) ),
        br(),
        br(),
        tags$style("#step2 {border-color:black}"),
        actionButton(inputId="step2", label=div("Step 2: apply model",
                                                tags$i(class="far
                                                       fa-question-circle")) ),
        br(),
        br(),
        tags$style("#step3 {border-color:black}"),
        actionButton(inputId="step3", label=div("Step 3: results",
                                                tags$i(class="far
                                                       fa-question-circle")) ),
        br(),
        br(),
        selectInput(inputId="prolifSButt", label=div(icon("vial"),
                                                     "Sample"),
                    choices=c("")),
    ...
    tabPanel(
      ...
      br(),
      sidebarPanel(
        selectInput(inputId="tSNEMode", label=div(icon("laptop-code"), "Mode"),
                    choices=c("Heatmap", "Overlay Groups or Samples",
                              "Overlay Populations")),
        selectInput(inputId="tSHighl", label=div(icon("highlighter"),
                                                 "Highlight"),
                    choices=c("")),
        selectInput(inputId="tSGroupOrSamp", label=div(icon("vial"),
                                                       "Events to show"),
                    choices=c("All", "Group", "Sample")),
        selectInput(inputId="tSGroupOrSampID", label=div(icon("vial"),
                                                         "Group/Sample"),
                    choices=c("")),
        actionButton(inputId="tSGroupOrSampIDs", label="Groups/Samples",
                     icon("plus-square")),
        actionButton(inputId="tSNEPopulations", label="Populations",
                     icon("plus-square")),
        ...
        sliderInput("tSNEGroups", label="Number of groups", min=1, max=6,
        ...
        tags$style(HTML("[for=tSNEDotSize]+span>.irs>.irs-single,
                        ...
                        border-top: black; border-bottom: black}")),
        sliderInput("tSNEDotSize", label="Dot size", min=2, max=18, step=1,
                    value=8, ticks=FALSE),
        selectInput(inputId="tSPar", label=div(icon("sitemap"),
                                               "Parent"),
                    choices=c("", "ungated")),
        actionButton(inputId="tSNEParameters", label="Select parameters",
                     icon("plus-square")),
        br(),
        br(),
        selectInput(inputId="tSEvs", label=div(icon("ellipsis-v"),
                                               "Events per sample"),
      ...
      mainPanel(
        ...
        div(style="display:inline-block",
            downloadButton(outputId="savetSNEPlot", label="Export image")),
        div(style="display:inline-block;vertical-align:top; width: 20px;",
            HTML("<br>")),
    ...
    tabPanel(
      ...
      br(),
      sidebarPanel(
        selectInput(inputId="rsStat", label=div(icon("percentage"),
                                                "Statistic"),
                    choices=c("", "Freq. of parent", "Freq. of...",
                              "Freq. of total", "Median", "Count")),
        selectInput(inputId="rsParent", label=div(icon("sitemap"),
                                                  "Parent"), choices=c("")),
        selectInput(inputId="rsPop", label=div(tags$i(class="fas fa-clone"),
                                               "Population"), choices=c("")),
        selectInput(inputId="rsCh", label=div(
          img(src=dataURI(file=system.file("icons", "X.png", package="BCyto"),
                          mime="image/png"), height="15px"),
                                              "Parameter"),
                    choices=c("")),
        tags$style("#addResult {border-color:black}"),
        actionButton(inputId="addResult", label="Add to table",
                     icon("plus-square")),
...
server <- function(input, output, session) {
  ...
  observeEvent(reactAxisCustom$d, {
    ...
    if(length(ff) > 0) {
      ...
      showModal(modalDialog(
        tags$style(HTML(paste0("[for=posslider]+span>.irs>.irs-single,
                                   [for=posslider]", bc$sliderCol))),
        tags$style(HTML(paste0("[for=negslider]+span>.irs>.irs-single,
                                   [for=negslider]", bc$sliderCol))),
        tags$style(HTML(paste0("[for=widthslider]+span>.irs>.irs-single,
        ...
        fluidRow(
          column(
            ...
            sliderInput("posslider", label="Positive Decades", min=2,
                        max=7, step=0.02, value=value[1], ticks=FALSE),
            sliderInput("negslider", label="Negative Decades", min=0,
                        max=1, step=0.1, value=value[2], ticks=FALSE),
            sliderInput("widthslider", label="Width Basis", min=0,
                        max=3, step=0.1, value=log10(abs(value[3])),
                        ticks=FALSE))),
      ...
      alert(paste0("The sample '", bc$fileList[index], "' has 0 events in
      the selected parent. Please choose another sample or
                   change the parent."))
  ...
  output$axisplot <- renderPlot({
    ...
    customAx <- c(input$posslider, input$negslider, as.numeric(bc$widthBasis))
  ...
  observeEvent(input$saveaxis, {
    ...
    newA <- c(input$posslider, input$negslider, as.numeric(bc$widthBasis))
        ...
        for(j in seq(gateCh)) {
          if(names(gate@parameters)[j] == reactAxisCustom$d) {
            if(is(gate, "polygonGate")) {
              obj <- inverseT(gate@boundaries[,reactAxisCustom$d])
  ...
  observeEvent(input$displayOptMain, {
    showModal(modalDialog(
      tags$style(HTML(paste0("[for=displayAxisFont]+span>.irs>.irs-single,
                             [for=displayAxisFont]", bc$sliderCol))),
      ...
      br(),
      tags$style(HTML(paste0("[for=displayGateFont]+span>.irs>.irs-single,
                             [for=displayGateFont]", bc$sliderCol))),
      ...
      footer=list(
        actionButton(inputId="reloadplotmodal", label="Ok", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel",
                     style=bc$whiteB)
      ),
      easyClose=TRUE,
  ...
  observeEvent(input$plotHelp, {
    showModal(modalDialog(
      title="Help",
      "Use the buttons at the top right side of the plot to interactively
      draw gates.",
      br(),
      br(),
      "Double clicking the population inside the gate leads to a new plot
      with the sorted population.",
      footer=NULL,
      size="w",
      easyClose=TRUE))
  })
  ...
  output$plotUI <- renderUI({
    if(input$typ == "Histogram") {
      ...
      if(reactRectangle$d == TRUE) {
        ...
        brushOpts <- NULL
        if(isolate(reactPolygon$d) == TRUE || reactQuadrant$d == TRUE) {
          ...
          for(i in seq(nrow(coords))) {
            if(i > 1) {
              segments(coords$x[i-1], coords$y[i-1],
                       coords$x[i], coords$y[i], col="red", lwd=2)
            }
            points(coords$x[i], coords$y[i], pch=19, cex=2, col="red")
          }
        }
      }
  ...
  observeEvent(c(input$rectang, input$polygon, input$quad, input$interval), {
    if(!is.null(bc$gs)) {
      ...
      if(length(bc$found) >= 4) {
        showModal(modalDialog(
          "Only 4 gates are allowed on a plot.
          Would you like to remove the 4 existing gates to draw new ones?",
          footer=list(
  ...
  observeEvent(input$main_dblclick, {
    ...
    if(length(popPaths) > 1) {
      ...
      popGates <- vapply(popPaths[-1], function(x)
        list(gs_pop_get_gate(bc$gs, x)[1][[1]]), list(length(popPaths[-1])))
      clickableGate <- list()
      for(i in seq(popGates)) {
  ...
  observeEvent(input$hierarchy_click, {
    ...
    selected <- nearPoints(dataF, input$hierarchy_click, xvar="x", yvar="y",
                           threshold=25, maxpoints=1, addDist=TRUE)
    if(length(rownames(selected)) > 0
       && rownames(selected) != "ungated") {
      reactNameChange$d <- rownames(selected)
      updateSelectInput(inputId="editGate",
                        label=HTML("<span style='color: red'>Edit</span>"))
      enable("editGate")
    } else {
      updateSelectInput(inputId="editGate", label="Edit")
      disable("editGate")
  ...
  observeEvent(input$editGate, {
    ...
    showModal(modalDialog(
      ...
      footer=list(
        if(substr(path, 1, 1) != "Q" || substr(path, 3, 4) != ": ") {
          actionButton(inputId="changeNameModal", label="Ok", style=bc$blackB)
        },
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB),
  ...
  observeEvent(input$deleteGate, {
    ...
    removeModal()
    updateSelectInput(inputId="editGate",
                      label=HTML("<span style='color: black'>Edit</span>"))
  })
    ...
    if(!is.null(bc$compControlIDs)){
      ...
      if(length(bc$compControlIDs) >= length(bc$fluoCh) && verifier) {
        ...
        withProgress(message=message, detail="", value=0, max=100, {
          ...
          spill = get.marker.spillover(TRUE, flowGate, flowControl, asp)
        ...
        alert("AutoSpill requires all compensation control files to be
              loaded.")
      }
    } else {
      alert("AutoSpill requires all compensation control files to be
  ...
  observeEvent(input$applyMatrix, {
    showModal(modalDialog(
      paste0("The matrix '", input$previewMatrix,
             "' will be applied to all samples. Do you want to proceed?"),
      footer=list(
        actionButton(inputId="OKapplymatrix", label="Ok", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
      ),
      easyClose=TRUE,
      size="s"))
    ...
    output$compGen <- renderPlot({
      ...
      if(isolate(reactReadOnly$d) == TRUE) {
        withProgress(message="Please wait...", detail="", value=0, max=100, {
          compGen(ID, bc$compDFs[input$previewMatrix][[1]],
                  input$showUncomp)
        })
      } else {
        withProgress(message="Please wait...", detail="", value=0, max=100, {
          compGen(ID, bc$currentTable/100, input$showUncomp)
        })
      }
  ...
  #Overlays----
  ##left----
  observeEvent(c(input$ovTyp, input$ovTon, input$ovY, input$ovX, input$ovP), {
    ...
    if(input$ovTyp == "Overlaid histogram"
      ...
      if(input$ovTyp == "Overlaid histogram") {
        if(input$ovX != "" && input$ovP != "") {
          ...
          showOvPlot$d <- FALSE }
        if(input$ovTon != "") {
          updateSelectInput(inputId="ovTon", selected="")
        } else {
          ovActivator$d <- ovActivator$d + 1
        }
        disable("ovTon")
      } else {
        if(input$ovTon != "" && input$ovX != "" && input$ovP != "") {
          enable("ovSamples")
          ovActivator$d <- ovActivator$d + 1
          showOvPlot$d <- TRUE
        } else {
      ...
      if(input$ovTyp != "" && input$ovTon != "" && input$ovY != ""
         && input$ovX != "" && input$ovP != "") {
        enable("ovSamples")
        ovActivator$d <- ovActivator$d + 1
        showOvPlot$d <- TRUE
  ...
  observeEvent(input$ovSamples, {
    ...
    showModal(modalDialog(
      ...
      strong(modaltitle),
      checkboxGroupInput("samplecheckbox", label="", choices=bc$sampleList,
                         width="100%" ),
      footer=fluidRow(
        ...
        column(width=4, align="right", list(
          actionButton(inputId="ovokbutton", label="Ok", style=bc$blackB),
          actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
        ),
        )
      ),
  ...
  observeEvent(input$displayOptOv, {
    ...
    showModal(modalDialog(
      ...
      footer=list(
        actionButton(inputId="ovreloadplotmodal", label="Ok", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
      ),
      easyClose=TRUE,
      size="s"))
    ...
    choices <-
      showModal(modalDialog(
        ...
        radioButtons("orderradiobutton", label="",
                     choices=rev(isolate(reactOvSamples$d)), width="100%",
                     selected="" ),
        footer=actionButton(inputId="cancelModal", label="Cancel",
                            style=bc$whiteB),
        easyClose=TRUE,
  ...
  observeEvent(input$step3, {
    showModal(modalDialog(
      title = "Step 3 help",
      "You can use the generated results or return to step 2 to apply
      a new model.",
      footer=list(
  ...
  output$prolifPlot <- renderPlot({
    ...
    if("undivided" %in% popPaths) {
      prolifGen(input$prolifSButt, isolate(reactShowAxis$d),
                isolate(reactAxisFont$d), prolifReady$d, input$prolifLabel,
                input$prolifGrid)
    } else {
      prolifOff()
      hide("prolifPlot")
  ...
  output$exportTableProlif <- downloadHandler(
    ...
    function(file) {
      colNames = c("Number of divisions", "Divided cells", "Undivided cells")
      for(i in 2:nrow(bc$refPeaks)) {
      ...
      for(i in 2:nrow(bc$refPeaks)) {
        colNames[i+(constantColLenght-1)] = paste0("Percent division ",
                                                   (i - 1))
      }
  ...
  observeEvent(input$tsneoksampbutton, {
    ...
    if(length(bc$concatSamples[[1]]) == 0) {
      ...
      for(i in seq(bc$concatSamples)) {
        bc$concatSamples[[i]] <- sapply(bc$concatSamples[[i]], function(x)
    ...
    if(length(grep(TRUE, unlist(lens))) > 0) {
      ...
      if(is.null(reactGroupNames$d)) {
        bc$concatSamples <- list()
        alert("Please type the name of each group in the corresponding
              text boxes.")
      } else {
  ...
  observeEvent(input$tSNEParameters, {
    ...
    showModal(modalDialog(
      ...
      checkboxGroupInput("tsneParCheckBox", label="", choices=choices,
                         width="100%"),
      footer=list(
        actionButton(inputId="tsneOkParButt", label="Ok", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
      ),
      easyClose=TRUE,
      size="m"))
  ...
  observeEvent(input$tSNEGenerate, {
    if(is.null(bc$entiretSNE)) {
      ...
      if(all(verifier)) {
        round <- round(length(unlist(bc$concatSamples))*events/1000*2.5/60, 1)
        minutes <- as.numeric(format(round, nsmall=1))
        showModal(modalDialog(
          "The t-SNE tool might require longer processing time than other
          tools in this software.",
          br(),
          br(),
          "Before proceeding, make sure that your CPU is not being used by
          heavy processes.",
          br(),
          br(),
          "Concatenation of the selected samples/number of events and t-SNE
          generation are estimated to take ",
          strong(minutes, "-", minutes*2, " minutes."),
          br(),
          br(),
          textOutput("question"),
          footer=list(
            actionButton(inputId="tSNEGenOk", label="Generate t-SNE",
                         style=bc$blackB),
            actionButton(inputId="cancelModal", label="Cancel",
                         style=bc$whiteB)),
          easyClose=FALSE,
        ...
        if(lowerNumber < 1000) {
          alert("There are not enough events (<1K) in at least one of the
                selected samples/parent. Try to change the samples
                or the Parent.")
        } else {
          if(lowerNumber < 5000) {
            maxEvents <- "1K"
          ...
          alert(paste("At least one selected samples/parent don't have
                      enough events. For the selected samples/parent, only",
                      maxEvents, "events are possible."))
        }
      }
    } else {
      showModal(modalDialog(
  ...
  observeEvent(input$tSNEGenOk, {
    ...
    for(i in seq(bc$fluoCh)) {
      trans <- flowjo_biexp(channelRange=4096, maxValue=262144,
                            pos=bc$customAxis[[i]][1],
                            neg=bc$customAxis[[i]][2],
                            widthBasis=bc$customAxis[[i]][3], inverse=TRUE)
    ...
    hide("cancelModal")
    hide("question")
    withProgress(message="Generating t-SNE...", detail="", value=0, max=100, {
      index <- seq_len(length(isolate(input$tsneParCheckBox)))
      df <- as.data.frame(bc$concat[,index])
      bc$entiretSNE <- Rtsne(df, check_duplicates=FALSE, verbose=TRUE)
    ...
    if(length(bc$tSNEListofGroups) < 2 && length(bc$tSNEListofSamples) < 2) {
      ...
      if(input$tSNEMode == "Heatmap"
        ...
        if(input$tSGroupOrSamp == "All") {
          ...
          if(input$tSGroupOrSamp == "Group") {
            ...
            updateSelectInput(inputId="tSGroupOrSampID",
                              choices=bc$tSNEListofGroups, selected=1)
            if(isolate(input$tSGroupOrSampID) == 1) {
              tSPlotActiv$d <- tSPlotActiv$d + 1
            }
          } else if(input$tSGroupOrSamp == "Sample") {
            updateSelectInput(inputId="tSGroupOrSampID",
                              choices=bc$tSNEListofSamples, selected=1)
            if(isolate(input$tSGroupOrSampID) == 1) {
              tSPlotActiv$d <- tSPlotActiv$d + 1
            }
          }
  ...
  observeEvent(input$tSGroupOrSampIDs, {
    ...
    showModal(modalDialog(
      ...
      footer=list(
        actionButton(inputId="tSNESetIDs", label="Ok", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
      ),
      easyClose=TRUE,
      size="m"))
  ...
  observeEvent(input$tSNEPopulations, {
    ...
    showModal(modalDialog(
      ...
      footer=list(
        actionButton(inputId="tSNESetPopIDs", label="Ok", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
      ),
      easyClose=TRUE,
      size="m"))
  ...
  output$tSNEPlot <- renderPlot({
    ...
    reacttSIDs$d
    if(!is.null(bc$entiretSNE)) {
      tSNEGen(input$tSNEDotSize, isolate(input$tSNEMode), reacttSNEPops$d,
              isolate(input$tSGroupOrSamp), isolate(input$tSGroupOrSampID),
              input$tSHighl, reacttSIDs$d, isolate(input$tsneIDs),
              reacttSNEPops$d)
  ...
  observeEvent(c(input$rsStat, input$rsPop), {
    if(!is.null(bc$gs)) {
      ...
      if(isolate(input$rsStat) == "Freq. of..."
         && isolate(input$rsPop) != "") {
        updateSelectInput(inputId="rsParent", choices=c(possibleParents))
        enable("rsParent")
      } else {
  ...
  observeEvent(input$addResult, {
    ...
    popStats <- gs_pop_get_count_fast(bc$gs, subpopulations=input$rsPop)
    id <- bc$onlySampleIDs
    if(input$rsStat == "Freq. of parent") {
      ...
      nums <- gs_pop_get_count_fast(bc$gs, "freq", subpopulations=input$rsPop)
      bc$results[[resultID]] <- sprintf("%.1f", nums$Count$Frequency*100)[id]
    } else if(input$rsStat == "Count") {
      bc$results[[resultID]] <- popStats[[4]][id]
    } else if(input$rsStat == "Freq. of...") {
      par <- input$rsParent
      parentstats <- gs_pop_get_count_fast(bc$gs, subpopulations=par)
  ...
  observeEvent(input$editResult, {
    choices <- setNames(as.list(seq(ncol(bc$results))), colnames(bc$results))
    showModal(modalDialog(
      "Select the rows to be deleted.",
      checkboxGroupInput("checkdelete", label="", choices=choices),
      footer=list(
        actionButton(inputId="deleteModal", label="Delete", style=bc$blackB),
        actionButton(inputId="cancelModal", label="Cancel", style=bc$whiteB)
      ),
      easyClose=TRUE,
      size="s")
    ...
    if(input$tabs == "compTab") {
      ...
      for(i in seq(bc$fluoCh)) {
        ...
        bc$uncompGSfortable <- transform(bc$uncompGSfortable,
                                         transformerList(bc$fluoCh[i], trans))
      }
      hide("saveMatrix")
      hide("cancelMatrix")
      enable("createMatrix")
      reactReadOnly$d <- TRUE
      bc$hTable$d <- as.data.frame(bc$compDFs["Cytometer-defined"][[1]]*100)
      output$comp <- renderRHandsontable({
        mat <- bc$compDFs[input$previewMatrix][[1]]*100
        visualtable <- rhandsontable(mat, rowHeaderWidth=100, stretchH="all",
  ...
  observeEvent(reactDir$d, {
    withProgress(message="Please wait...", detail="", value=0, max=100, {
      pars <- c(Home= path_home(), "R Installation"=R.home(), getVolumes())
      ...
      if(length(bc$tempFileList) == 0) {
        ...
        validityOut <- validityOut <- vapply(seq(bc$tempFileList), function(x){
          list(tryCatch({
          ...
          if(length(unique(channelLengths)) == 1) {
            ...
            if(all(verifier)) {
              ...
              for(i in seq(bc$tempFileList)) {
                ...
                tubeNam <- strsplit(tubeNameList[[i]], " ")[[1]]
                if(length(grep("Control", tubeNam)) > 0) {
                  if(length(grep("Stained", tubeNam)) > 0
                     || length(grep("Unstained", tubeNam)) > 0) {
              ...
              bc$finalSelect <- list(as.list(seq(bc$tempFileList)),
                                     as.list(seq(bc$tempFileList)))
              id <- bc$tempCompIDs
              if(!is.null(id)) {
                bc$tempOnlySampIDs <- seq(bc$tempFileList)[-id]
                bc$finalSelect[[1]][id] <- TRUE
                bc$finalSelect[[1]][-id] <- FALSE
                bc$finalSelect[[2]][bc$tempOnlySampIDs] <- TRUE
                bc$finalSelect[[2]][-bc$tempOnlySampIDs] <- FALSE
                if(length(bc$tempOnlySampIDs) == 0) {
                  bc$finalSelect[[2]][id] <- FALSE
                }
              } else {
                bc$finalSelect[[1]] <- lapply(bc$finalSelect[[1]],
                                              function(x) FALSE)
                bc$finalSelect[[2]] <- lapply(bc$finalSelect[[2]],
                                              function(x) TRUE)
              }
              sampleColum <- list()
              for(i in seq(bc$tempFileList)) {
              ...
              for(j in seq(2)) {
                for(i in seq(bc$tempFileList)) {
                  groupColums[[j]][[i]] <- checkboxInput(
                    ...
                    label="", value=bc$finalSelect[[j]][[i]])
                }
              }
              ...
              checkBoxColumns[[2]] <- column(
                width=2, align="center", style="margin-top: 10px;",
              ...
              showModal(modalDialog(
                ...
                fluidRow(
                  column(
                    width=6, align="right", list(sampleColum)),
                  checkBoxColumns),
                footer=list(
                  actionButton(inputId="okSelectCompAndSamp",
                               label="Ok", style=bc$blackB),
              ...
              verifier <- vapply(tubeList, function(x)
                !is.null(x), TRUE)
              if(all(tubeList)) {
                alert("Loading failed... At least one FCS file
                                  had its integrity altered. BCyto supports
                                  only original unmodified FCS files.")
              } else {
                alert("Loading failed... At least one FCS file
  ...
  observeEvent(input$okSelectCompAndSamp, {
    ...
    if(is.null(displayAlert)) {
      withProgress(message="Please wait...", detail="", value=0, max=100, {
        ...
        bc$compControlIDs <- seq(bc$fileList)[unlist(bc$finalSelect[[1]])]
        bc$onlySampleIDs <- seq(bc$fileList)[unlist(bc$finalSelect[[2]])]
        initializing()
        initInpUpd()
        plotActivator$d <- plotActivator$d + 1
        hierarchyActivator$d <- hierarchyActivator$d + 1
  ...
  observeEvent(input$loadTestData, {
    ...
    file1 <- "Cell maturation assay - 24 mb (FlowRepository ID FR-FCM-Z4LZ)"
    file2 <- "Proliferation assay - 18.5 mb (FlowRepository ID FR-FCM-Z4LY)"
    showModal(modalDialog(
  ...
  observeEvent(input$downloadTestData, {
    ...
    if(input$radioTestData == "test-data-1") {
      bc$fileList <- c("Compensation Controls_APC Stained Control_013.fcs",
      ...
      bc$fileList <- c("Compensation Controls_APC-Cy7 Stained Control_080.fcs",
      ...
      withProgress(message=message, detail="file 0", value=0, max=max, {
        for(i in seq(bc$fileList)) {
          ...
          download.file(paste0("https://github.com/BonilhaCaio/",
                               ...
                               adjustedFileList[i]),
                        destfile=file.path(bc$filePath, bc$fileList[i]))
        }
        download.file(paste0("https://github.com/BonilhaCaio/",
                             input$radioTestData, "/raw/main/",
                             datasetFile),
  ...
  observeEvent(reactBCytoFile$d, {
    ...
    if(all(verifier)) {
      ...
      withProgress(message="Please wait...", detail="", value=0, max=100, {
        ...
        if(reactBCytoFile$d[[1]] == "dataset-1.RData"
           || reactBCytoFile$d[[1]] == "dataset-2.RData") {
          load(paste0(bc$filePath, "/", reactBCytoFile$d), bc)
        } else {
          load(paste0(bc$filePath, "/", tempFilePath[[1]]), bc)
        }
        bc$sampleList <- bc$fileList[bc$onlySampleIDs]
        bc$uncompGS <- GatingSet( load_cytoset_from_fcs(bc$fileList))
        bc$gs <- gs_clone(bc$uncompGS)
        compensate(bc$gs, bc$compDFs[bc$appliedMatrix][[1]])
        indices <- grepl("SC", colnames(bc$gs)) != TRUE
        preFluoChannels <- colnames(bc$gs)[indices]
        bc$fluoCh <- preFluoChannels[colnames(bc$gs)[indices] != "Time"]
        completeFluoCh <- unlist(bc$ch)
        ...
        for(i in seq(bc$popGates)) {
          gs_pop_add(bc$gs, bc$popGates[[i]], parent=bc$popParents[i])
        }
        recompute(bc$gs)
        ...
        updateSelectInput(inputId="bgPreviewSample", choices=choices,
                          selected=bc$bgPreviewSample)
        updateSelectInput(inputId="bgType", selected=bc$bgType)
        updateSelectInput(inputId="bgPop", choices=c("", pops[-1]),
                          selected=bc$bgPop)
        showOvPlot$d <- bc$ovBoolean
        updateSelectInput(inputId="ovTyp", selected=bc$ovTyp)
        updateSelectInput(inputId="ovTon", selected=bc$ovTon)
        updateSelectInput(inputId="ovY", choices=c(bc$ch), selected=bc$ovY)
        updateSelectInput(inputId="ovX", choices=c(bc$ch), selected=bc$ovX)
        updateSelectInput(inputId="ovP", choices=pops, selected=bc$ovP)
        reactOvSamples$d <- bc$ovSamples
        reactOvShowAxis$d <- bc$ovShowAxis
        reactOvAxisFont$d <- bc$ovAxisFont
        prolifReady$d <- bc$prolifBool
        updateSelectInput(inputId="prolifSButt", choices=bc$sampleList,
        ...
        updateSelectInput(inputId="tSEvs", selected=bc$tSEvs)
        ...
        updateSliderInput(inputId="tSNEDotSize", value=bc$tSNEDotSize)
        if(!is.null(bc$entiretSNE)) {
          showtSNE()
          tempChannels <- unlist(bc$ch)[unlist(bc$ch) != "Time"]
          bc$availableparameters <- list()
          for(i in seq(bc$tSNEParameters)) {
            index <- which(tempChannels == bc$tSNEParameters[i])
            bc$availableparameters[[i]] <- index
          }
          indices <- unlist(bc$availableparameters)
          bc$availableparameters <- tempChannels[indices]
          choices <- c(bc$availableparameters)
          updateSelectInput(inputId="tSHighl", choices=choices)
          tSPlotActiv$d <- tSPlotActiv$d + 1
        } else {
          hidetSNE()
          enable("tSNEGenerate")
  ...
  observeEvent(input$fileHelp, {
    showModal(modalDialog(
      ...
      br(),
      strong("Important:"),
      "BCyto files are required to be inside the loaded FCS files folder
